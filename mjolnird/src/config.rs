...
mod tests {
    ...
    fn it_builds_a_master_config() {
        let args = Config::matches().get_matches_from(vec!["mjolnird", "--bind=192.168.0.101:11011", "master"]);
        let config = Config::from_args(args);
        assert_eq!(config.mode, Mode::Master);
    ...
    fn it_builds_a_default_agent_config() {
        let args = Config::matches().get_matches_from(vec!["mjolnird", "agent", "--master=192.168.0.100:11011"]);
        let config = Config::from_args(args);
        assert_eq!(config.mode, Mode::Agent(vec!["192.168.0.100:11011".parse().unwrap()]));
        assert_eq!(config.bind_address, "0.0.0.0:11012".parse().unwrap());
    }

    #[test]
    fn it_builds_an_agent_config() {
        let args = Config::matches().get_matches_from(vec!["mjolnird", "--bind=192.168.0.101:11012", "agent", "--master=192.168.0.100:11011"]);
        let config = Config::from_args(args);
        assert_eq!(config.mode, Mode::Agent(vec!["192.168.0.100:11011".parse().unwrap()]));
